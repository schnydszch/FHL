...
<xsl:if test="$DisplayOPACiconsXSLT!='0'">
    ...
    <xsl:if test="string-length(normalize-space($physicalDescription))">
        ...
        <xsl:if test="$typeOf008='BK'">
			...
			<!-- Commenting out the "Literary Form" code. Modified for Filipinas Heritage Library
        ...
        <xsl:if test="$typeOf008='MU' and $controlField008-30-31 and $controlField008-30-31!='||' and $controlField008-30-31!='  '">
            <span class="label"> </span> <!-- Literary text for sound recordings modified this for Filipinas Heritage Library; removed "; Literary form:  -->
				  ...
				  <xsl:attribute name="href"><xsl:value-of select="marc:subfield[@code='u']"/></xsl:attribute>
				   ...
				   <xsl:when test="count(key('item-by-status', 'available'))=0 and count(key('item-by-status', 'reference'))=0">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>Ask library staff for availability</xsl:otherwise>
                        </xsl:choose>
				   </xsl:when>
                   <xsl:when test="count(key('item-by-status', 'available'))>0">
                   <span class="available">
                       <b><xsl:text>Copies available: </xsl:text></b>
